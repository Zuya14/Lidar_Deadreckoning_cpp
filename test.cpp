...
void test2() {
	...
	while (!read_file.eof()) {
		...
		if (points.size() == 1081) {
			...
			// bool success = lrfDR.update(points); 
			std::pair<Eigen::Matrix2d, Eigen::Vector2d> RT = icp.estimate(points, lidarMap0, R.transpose(), -T, 50, 1e-3);
            R = RT.first;
            T = RT.second;
            ...
            std::pair<Eigen::Matrix2d, Eigen::Vector2d> RT2 = icp.estimate2(points, lidarMap0, R2.transpose(), -T2, 50, 1e-3);
            R2 = RT2.first;
            T2 = RT2.second;

            std::tuple<double, double, double> xyt2 = icp.calc_XY_Theta(R2, T2);
            double x2 =  std::get<0>(xyt2);
            double y2 =  std::get<1>(xyt2);
            double theta2 =  std::get<2>(xyt2);
            
            end2 = std::chrono::system_clock::now();

			double time = static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() / 1000.0);
			double time2 = static_cast<double>(std::chrono::duration_cast<std::chrono::microseconds>(end2 - start2).count() / 1000.0);
            
            // printf("(%d, %d, %1.2f), %f[ms]\n", (int)x, (int)y, theta, time);
            printf("(%.2f, %.2f, %1.2f), %.2f[ms]     ", x,  y,  theta,  time);
            printf("(%.2f, %.2f, %1.2f), %.2f[ms]\n",    x2, y2, theta2, time2);
            // printf("%f[ms]\n\n", time);
            sum_time += time;
            count++;
		}
